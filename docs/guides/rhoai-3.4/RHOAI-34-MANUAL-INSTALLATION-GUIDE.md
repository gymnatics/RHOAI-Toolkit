# Red Hat OpenShift AI 3.4 -- Manual Installation Guide

This guide covers step-by-step manual installation of Red Hat OpenShift AI (RHOAI) 3.4
on OpenShift Container Platform 4.19+. It provides two installation paths depending on
whether you need Models-as-a-Service (MaaS) or direct model serving only.

> **Automated alternative:** Run `scripts/install-rhoai-34.sh` for a fully automated
...
RHOAI 3.4 supports two deployment topologies. Choose the path that matches your needs.
...
| Rate Limiting | None | Token-based via Limitador |
| Extra Operators | None | RHCL, Service Mesh 3, Kueue, LWS |
...
> **Note:** Distributed inference with llm-d (Path B) works best on OCP 4.20+.
...
# RHOAI 3.4 requires OCP 4.19+
if [[ "$OCP_VERSION" < "4.19" ]]; then
  echo "ERROR: RHOAI 3.4 requires OpenShift 4.19 or later"
  exit 1
fi
```
...
- `stable-3.4` -- Pinned to 3.4 stream
...
> **Key difference from Path B:** `modelsAsService` is set to `Removed`.
...
### 5B.3 OpenShift Service Mesh 3

Service Mesh 3 (Sail/Istio) is required by RHCL for the API gateway.
...
until oc get csv -n openshift-operators 2>/dev/null | grep -q "servicemeshoperator3.*Succeeded"; do
  ...
  # Re-approve any new pending InstallPlans
  for PLAN in $(oc get installplan -n openshift-operators --no-headers 2>/dev/null | awk '{print $1}'); do
    APPROVED=$(oc get installplan "$PLAN" -n openshift-operators -o jsonpath='{.spec.approved}')
    CSVS=$(oc get installplan "$PLAN" -n openshift-operators -o jsonpath='{.spec.clusterServiceVersionNames[*]}')
    if [ "$APPROVED" = "false" ] && echo "$CSVS" | grep -qiE "servicemesh|kiali|sail"; then
...
and Gateway API integration. RHOAI 3.4 MaaS requires RHCL v1.2+.

> **Warning:** RHCL v1.4.0 has known WASM plugin loading bugs that break MaaS.
> The manifest pins to v1.3.4 via `installPlanApproval: Manual`.
...
```bash
echo "Waiting for RHCL InstallPlan..."
until oc get subscription rhcl-operator -n openshift-operators \
  -o jsonpath='{.status.installPlanRef.name}' &>/dev/null 2>&1; do
  sleep 5
done

# Approve all RHCL-related InstallPlans (RHCL + dependencies: Authorino, DNS, Limitador)
for PLAN in $(oc get installplan -n openshift-operators --no-headers 2>/dev/null | awk '{print $1}'); do
  APPROVED=$(oc get installplan "$PLAN" -n openshift-operators -o jsonpath='{.spec.approved}')
  CSVS=$(oc get installplan "$PLAN" -n openshift-operators -o jsonpath='{.spec.clusterServiceVersionNames[*]}')
  if [ "$APPROVED" = "false" ] && echo "$CSVS" | grep -qiE "rhcl|authorino|limitador|dns-operator"; then
    echo "Approving RHCL InstallPlan: $PLAN ($CSVS)"
    oc patch installplan "$PLAN" -n openshift-operators --type merge -p '{"spec":{"approved":true}}'
  fi
done
```
...
as specified by the RHOAI 3.4 MaaS documentation.
...
RHOAI 3.4 uses OpenShift service-ca for Authorino TLS (not cert-manager Certificate
...
> **Key difference from Path A:** This manifest sets `modelsAsService: Managed`
> under `kserve`, enabling the MaaS controller, MaaS CRDs, and the Tenant
> auto-creation in the `models-as-a-service` namespace.
...
while [ $ELAPSED -lt 300 ]; do
  PHASE=$(oc get datasciencecluster default-dsc -o jsonpath='{.status.phase}' 2>/dev/null)
  if [ "$PHASE" = "Ready" ]; then
    echo "DataScienceCluster is fully ready"
    break
  fi

  DASHBOARD=$(oc get datasciencecluster default-dsc \
    -o jsonpath='{.status.conditions[?(@.type=="DashboardReady")].status}' 2>/dev/null)
  KSERVE=$(oc get datasciencecluster default-dsc \
    -o jsonpath='{.status.conditions[?(@.type=="KserveReady")].status}' 2>/dev/null)

  if [ "$DASHBOARD" = "True" ] && [ "$KSERVE" = "True" ]; then
    echo "Core components ready (Dashboard, KServe). MaaS/Kueue may need further steps."
    break
  fi

  echo "  DSC phase: $PHASE (${ELAPSED}s)"
  sleep 15
  ELAPSED=$((ELAPSED + 15))
done
```
...
### 10B.1 Generate Database Credentials
...
### 10B.2 Resolve PostgreSQL Image
...
### 10B.3 Deploy PostgreSQL

```bash
oc apply -n redhat-ods-applications -f lib/manifests/maas/postgres-pvc.yaml
oc apply -n redhat-ods-applications -f lib/manifests/maas/postgres-service.yaml

export PG_IMAGE PG_USER PG_PASSWORD PG_DB
envsubst '${PG_IMAGE} ${PG_USER} ${PG_PASSWORD} ${PG_DB}' \
  < lib/manifests/maas/postgres-deployment.yaml | oc apply -n redhat-ods-applications -f -
```
...
DB_URL="postgresql://${PG_USER}:${ENCODED_PASSWORD}@postgres:5432/${PG_DB}?sslmode=disable"
...
```bash
echo "=== PostgreSQL ==="
echo "PVC:            $(oc get pvc postgres-data -n redhat-ods-applications -o jsonpath='{.status.phase}' 2>/dev/null)"
echo "Deployment:     $(oc rollout status deployment/postgres -n redhat-ods-applications --timeout=5s 2>&1 | tail -1)"
echo "maas-db-config: $(oc get secret maas-db-config -n redhat-ods-applications &>/dev/null && echo 'exists' || echo 'MISSING')"
echo "postgres-creds: $(oc get secret postgres-creds -n redhat-ods-applications &>/dev/null && echo 'exists' || echo 'MISSING')"
```
...
| `maasAuthPolicies` | MaaS admin features (subscriptions, auth policies) | Required (GA) |
...
```bash
oc get odhdashboardconfig odh-dashboard-config -n redhat-ods-applications \
  -o jsonpath='{.spec.dashboardConfig}' | jq '{modelAsService, genAiStudio, maasAuthPolicies, vLLMDeploymentOnMaaS}'
```
...
Five MaaS CRDs should be installed:
...
echo "Found: $MAAS_CRD_COUNT (expected: 5)"
```
...
The MaaS controller auto-creates a `default-tenant` in the `models-as-a-service`
namespace:
...
> secret exists in `redhat-ods-applications`. If it shows `Degraded`, verify
> the database secret.

### 12B.3 Gateway Annotations
...
### 12B.6 Full Platform Status

```bash
echo "============================================================"
echo "  MaaS Platform Status Summary"
echo "============================================================"
echo ""
echo "Operators:"
...
echo "  DB Config:    $(oc get secret maas-db-config -n redhat-ods-applications &>/dev/null && echo 'exists' || echo 'MISSING')"
...
HTTP 429 indicates rate limiting is working.
...
### RHCL InstallPlan Stuck

RHCL uses `installPlanApproval: Manual`. Check for unapproved plans:
...
Service Mesh 3 also uses Manual approval. Check and approve:
...
| cert-manager | KServe, RHCL | `cert-manager-operator` |
...
| Service Mesh 3 | RHCL/Istio gateway | `openshift-operators` |
| RHCL | Kuadrant, Authorino, Limitador | `openshift-operators` |
| Kuadrant | MaaS auth + rate limiting | `kuadrant-system` |
| Istio | Gateway data plane | `istio-system` |
| PostgreSQL | MaaS API keys | `redhat-ods-applications` |
...
5. RHOAI Operator
6. DSCInitialization
7. DataScienceCluster (modelsAsService: Removed)
8. Dashboard patch
9. Hardware Profile
...
```
 ...
 5. Kueue
 6. LWS
 7. Service Mesh 3 (approve InstallPlans)
 8. RHCL (approve InstallPlans)
 9. Kuadrant
10. IstioCNI + Istio
11. GatewayClasses
12. Authorino TLS (service-ca)
13. Gateway resource ConfigMap
14. Gateway (maas-default-gateway)
15. Gateway TLS secret
16. Passthrough Routes
17. RHOAI Operator
18. DSCInitialization
19. DataScienceCluster (modelsAsService: Managed)
20. PostgreSQL + maas-db-config secret
21. Dashboard patch (MaaS flags)
22. LLMInferenceService + MaaS CRs
```
...
| `redhat-ods-applications` | Dashboard, model serving, PostgreSQL |
| `redhat-ods-monitoring` | RHOAI monitoring |
| `kuadrant-system` | Kuadrant, Authorino |
| `openshift-ingress` | Gateway, Routes |
| `istio-system` | Istio control plane |
| `istio-cni` | Istio CNI |
| `models-as-a-service` | MaaS Tenant, subscriptions, auth policies |
...
| `postgres-creds` | `redhat-ods-applications` | PostgreSQL credentials |
...
| | `rhoai/datasciencecluster-v3-34.yaml` | -- |
| | `rhoai/hardware-profile-gpu.yaml` | -- |
| | `rhoai/mlflow-cr.yaml` | -- |
| **RHCL** | `rhcl/rhcl-operator-34.yaml` | -- |
