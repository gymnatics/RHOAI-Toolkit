...
openshift-install version     # OpenShift Installer (클러스터 신규 설치 시 필수)
...
> - **openshift-install** (클러스터 신규 설치 시 필수):
>   ```bash
>   # macOS Apple Silicon (M1/M2/M3/M4)
>   curl -L https://mirror.openshift.com/pub/openshift-v4/clients/ocp/latest/openshift-install-mac-arm64.tar.gz -o openshift-install.tar.gz
>   # macOS Intel
>   # curl -L https://mirror.openshift.com/pub/openshift-v4/clients/ocp/latest/openshift-install-mac.tar.gz -o openshift-install.tar.gz
>   # Linux
>   # curl -L https://mirror.openshift.com/pub/openshift-v4/clients/ocp/latest/openshift-install-linux.tar.gz -o openshift-install.tar.gz
...
# SSH agent에 키 등록
eval $(ssh-agent)
ssh-add ~/.ssh/id_ed25519    # 또는 ~/.ssh/id_rsa
```
...
RHOAI 3.x (Current):
1) Complete Setup (OpenShift + RHOAI 3.x + GPU + MaaS) [Full]   ← 1번 선택
2) Minimal RHOAI 3.x Setup (choose operators) [Flexible]
3) Install RHOAI 3.x [Recommended]

Management & Tools:
9) Configure Kubeconfig [Connection]
```

**지금 할 일:** 클러스터가 없으므로 **`1`을 선택**합니다. OpenShift 클러스터 설치 + GPU + RHOAI를 한 번에 진행합니다.

> **`openshift-install` 바이너리가 없어도 괜찮습니다!**
> 스크립트가 자동으로 감지하여 다운로드 옵션을 제안합니다.
> "1) Download automatically now (recommended)" 를 선택하면 버전 선택 후 자동 다운로드됩니다.

> **사전 필수:** `oc`, `aws` CLI, SSH 키가 설정되어 있어야 합니다 (Step 0 참고).

> **이미 클러스터가 있다면?** → [Step 2](#step-2-클러스터-연결-확인)로 이동 후, [Step 3](#step-3-rhoai-34-설치)에서 **3번 (Install RHOAI 3.x)** 을 선택하세요.
