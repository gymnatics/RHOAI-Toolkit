...
restart_kuadrant_operator() {
    ...
    # RHOAI 3.5 / RHCL 1.4: use oc wait for Kuadrant Ready condition
...
install_rhcl_operator() {
    # RHOAI 3.5 MaaS prerequisite (Govern LLM access with Models-as-a-Service):
    #   "installed the Red Hat Connectivity Link Operator version 1.4.1 or later
    ...
    #
    # RHCL 1.4.0 is deprecated: OCP clusters running 1.4.0 may see auth failures,
    # API key errors, gateway instability, or gateway pod memory pressure. Always use
    # 1.4.1 or later (the `stable` channel picks this up automatically once GA).
    # RHCL 1.4 requires OpenShift Service Mesh 3.4 if Service Mesh is used explicitly.
    #
    # Note: RHCL's own docs put everything in kuadrant-system. Both patterns work
    ...
    print_step "Installing Red Hat Connectivity Link (RHCL) v1.4.1+ Operator..."
...
setup_maas_database() {
    ...
    # RHOAI 3.5: maas-db-config secret goes in the infrastructure namespace
    #   (redhat-ai-gateway-infra), NOT redhat-ods-applications as in 3.4.
    # Reference: https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.5/html/govern_llm_access_with_models-as-a-service
    print_step "Setting up MaaS PostgreSQL database..."
    print_info "Infrastructure namespace: $MAAS_INFRA_NS"

    # Ensure the infrastructure namespace exists (created by maas-controller, but we may
    # need it before the controller finishes bootstrapping)
    oc create namespace "$MAAS_INFRA_NS" 2>/dev/null || true

    if oc get secret maas-db-config -n "$MAAS_INFRA_NS" &>/dev/null; then
        print_success "maas-db-config secret already exists in $MAAS_INFRA_NS"
        return 0
    fi
    ...
    while [ $elapsed -lt 120 ]; do
        if oc rollout status deployment/postgres -n "$MAAS_INFRA_NS" --timeout=5s &>/dev/null; then
    ...
    local db_url="postgresql://${pg_user}:${encoded_password}@postgres.${MAAS_INFRA_NS}.svc.cluster.local:5432/${pg_db}?sslmode=disable"
    ...
    oc create secret generic postgres-creds \
        ...
        -n "$MAAS_INFRA_NS" --dry-run=client -o yaml | \
        oc apply -n "$MAAS_INFRA_NS" -f -

    print_success "POC PostgreSQL deployed and maas-db-config secret created in $MAAS_INFRA_NS"
...
configure_maas_tls() {
    # RHOAI 3.5 MaaS TLS uses OpenShift service-ca (NOT cert-manager) - unchanged from 3.4
    ...
    # RHOAI 3.5: Wait for Authorino deployment to be Available after TLS config
    ...
    oc wait --for=condition=Available deployment/authorino -n kuadrant-system --timeout=300s 2>/dev/null || \
        print_warning "Authorino deployment not yet Available"
...
configure_maas_rate_limiting() {
    # RHOAI 3.5 note: maas-controller auto-creates maas-gateway-auth (AuthPolicy) and
    # gateway-default-deny (TokenRateLimitPolicy) — no need to create rate-limit policies
    # manually. The Redis + EnvoyFilter fixes below are Istio-level infrastructure that
    # complements (not conflicts with) the auto-created MaaS policies.
    # RHCL 1.4.1+ fixes the WASM-related memory pressure bugs, so the EnvoyFilter
    # workarounds may no longer be strictly necessary, but they're still safe to apply.
    print_step "Configuring MaaS token rate limiting infrastructure (Redis + EnvoyFilters)..."
...
create_inference_gateway() {
    ...
    oc wait --for=condition=Programmed gateway/maas-default-gateway -n openshift-ingress --timeout=120s 2>/dev/null || \
        print_warning "maas-default-gateway not yet Programmed"
...
verify_maas_deployment() {
    print_step "Verifying MaaS deployment (RHOAI 3.5)..."

    # RHOAI 3.5: Check ModelsAsAServiceReady DSC condition (note: "AsA" not "As")
    # Its message tells you exactly what is missing (gateway, DB secret, Authorino TLS)
    local maas_ready=$(oc get datasciencecluster default-dsc \
        -o jsonpath='{.status.conditions[?(@.type=="ModelsAsAServiceReady")].status}' 2>/dev/null)
    local maas_msg=$(oc get datasciencecluster default-dsc \
    ...
    # Check MaaS CRDs (3.5 adds maastenantconfigs, aitenants, tokenratelimitpolicies)
    ...
    if [ "$maas_crds" -ge 5 ]; then
        ...
        print_info "Expected CRDs: maassubscriptions, maasauthpolicies, maasmodelrefs, maastenantconfigs, tenants, aitenants, externalmodels"
    fi

    # Check MaasTenantConfig CR (3.5 uses MaasTenantConfig, not the legacy Tenant CRD)
    ...
    # Check maas-db-config secret (3.5: in infrastructure namespace, not redhat-ods-applications)
    print_step "Checking maas-db-config secret in $MAAS_INFRA_NS..."
    if oc get secret maas-db-config -n "$MAAS_INFRA_NS" &>/dev/null; then
        local has_url=$(oc get secret maas-db-config -n "$MAAS_INFRA_NS" \
            -o jsonpath='{.data.DB_CONNECTION_URL}' 2>/dev/null)
        if [ -n "$has_url" ]; then
            print_success "maas-db-config secret exists with DB_CONNECTION_URL in $MAAS_INFRA_NS"
        else
        ...
        print_info "Create it: oc create secret generic maas-db-config \\"
        print_info "  --from-literal=DB_CONNECTION_URL='postgresql://user:pass@host:5432/db?sslmode=require' \\"
        print_info "  -n $MAAS_INFRA_NS"
    fi

    # Check MaaS deployments
    # maas-api is in the infrastructure namespace; maas-controller is in redhat-ods-applications
    print_step "Checking MaaS deployments..."
    for deploy_spec in "maas-api:$MAAS_INFRA_NS" "maas-controller:redhat-ods-applications" "ai-gateway-operator:redhat-ods-applications"; do
        local deploy="${deploy_spec%%:*}"
        local ns="${deploy_spec##*:}"
        if oc get deployment "$deploy" -n "$ns" &>/dev/null; then
            local ready=$(oc get deployment "$deploy" -n "$ns" \
            ...
            print_info "  $deploy not found in $ns (may still be starting)"
        fi
    done
    ...
    # Check auto-created gateway policies (3.5: maas-controller creates these automatically)
    # Actual names from live cluster: maas-gateway-auth, gateway-default-deny
    print_step "Checking auto-created gateway policies..."
    local auth_policies=$(oc get authpolicy -n openshift-ingress --no-headers 2>/dev/null | wc -l | tr -d ' ')
    if [ "${auth_policies:-0}" -gt 0 ]; then
        print_success "  AuthPolicy in openshift-ingress: $auth_policies found"
        oc get authpolicy -n openshift-ingress --no-headers 2>/dev/null | while read -r line; do
    ...
    if oc get tokenratelimitpolicy -n openshift-ingress --no-headers &>/dev/null 2>&1; then
        local trl_count=$(oc get tokenratelimitpolicy -n openshift-ingress --no-headers 2>/dev/null | wc -l | tr -d ' ')
    ...
    # Match exact JSON {"status":"healthy"} — do NOT match "no healthy upstream"
    ...
    if echo "$maas_health" | grep -q '"status":"healthy"'; then
        print_success "MaaS API health: $maas_health"
    elif echo "$maas_health" | grep -q "no healthy upstream"; then
        print_warning "MaaS API: no healthy upstream (maas-api may still be starting or not ready)"
        print_info "Check: oc get pods -n $MAAS_INFRA_NS"
    elif [ -n "$maas_health" ]; then
        print_warning "MaaS API health endpoint returned: $maas_health"
    else
        print_info "MaaS API health endpoint not reachable yet"
    fi
    ...
    # /v1/api-keys/* calls (symptom: dashboard API Keys page shows "unknown
    # error when invoking maas-api (unmarshal): invalid character" because the
    # WASM shim returns the error text instead of JSON). Restarting the gateway
    # after maas-api has stabilized clears the stale state.
    ...
    if oc delete pod -n openshift-ingress \
        ...
        # Verify MaaS API health post-restart
        sleep 5
        local post_health
        post_health=$(curl -sk "https://maas.apps.${CLUSTER_DOMAIN}/maas-api/health" 2>/dev/null || true)
        if echo "$post_health" | grep -q '"status":"healthy"'; then
            print_success "MaaS API healthy after gateway restart"
        else
