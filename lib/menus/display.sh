...
show_main_menu() {
    ...
    echo -e "${YELLOW}2)${NC} Workshop Demo Setup (RHOAI 3.4 + OpenWebUI) ${GREEN}[Workshops]${NC}"
    echo ""
...
show_rhoai_management_menu() {
    ...
    echo "    Setup MaaS, LlamaStack/OGX, MCP Servers, Feature Store"
    echo ""
    echo -e "${YELLOW}3)${NC} Demos ${GREEN}[Version-Aware]${NC} ${BLUE}→${NC}"
    echo "    Deploy ready-to-use demos (Banking, LlamaStack/OGX, GuideLLM)"
    echo ""
    echo -e "${YELLOW}4)${NC} RHOAI 3.2+ Features ${GREEN}[NEW]${NC} ${BLUE}→${NC}"
    echo "    llm-d, MLflow, Observability (per CAI Guide)"
    echo ""
    echo -e "${YELLOW}5)${NC} Dashboard & Configuration ${BLUE}→${NC}"
    echo "    Enable GA/TP features (AutoML, AutoRAG, Observability, etc.)"
    echo ""
...
show_ai_services_submenu() {
    ...
    echo ""
    echo -e "${MAGENTA}LlamaStack / OGX:${NC} ${GREEN}[Version-Aware]${NC}"
    echo -e "${YELLOW}2)${NC} Setup LlamaStack/OGX (Generic)"
    echo "    Deploy LlamaStack (3.4-) or OGX (3.5+) with vLLM, Azure, OpenAI, Ollama, or Bedrock"
    echo ""
    echo -e "${YELLOW}3)${NC} Enable LlamaStack/OGX Operator"
    echo "    Enable the LlamaStack (3.4-) or OGX (3.5+) operator in DataScienceCluster"
    echo ""
    ...
    echo -e "${YELLOW}8)${NC} Configure MaaS Telemetry"
    echo "    Enable usage metrics capture (group, model, organization, user)"
    echo ""
    echo -e "${YELLOW}9)${NC} Tag MaaS Subscription Metadata"
...
show_dashboard_config_submenu() {
    ...
    echo -e "${YELLOW}1)${NC} Enable GA Dashboard Features"
    echo "    Model Registry, GenAI Studio, MaaS, MCP Catalog, etc."
    echo ""
    echo -e "${MAGENTA}Technology Preview:${NC}"
    echo -e "${YELLOW}2)${NC} Enable Technology Preview Features ${GREEN}[Interactive]${NC}"
    echo "    AutoML, AutoRAG, Guardrails, Observability, Tracing, etc."
    echo ""
    echo -e "${MAGENTA}Status:${NC}"
    echo -e "${YELLOW}3)${NC} Show Current Dashboard Config"
    echo "    View all OdhDashboardConfig flags and their values"
    echo ""
    echo -e "${YELLOW}0)${NC} Back to RHOAI Management"
    echo ""
}
...
show_demos_submenu() {
    ...
    echo ""
    echo -e "${YELLOW}3)${NC} Deploy LlamaStack/OGX Demo ${GREEN}[Version-Aware]${NC}"
...
show_rhoai32_features_submenu() {
    ...
    echo ""
    echo -e "${YELLOW}4)${NC} Enable LlamaStack/OGX Operator ${GREEN}[Version-Aware]${NC}"
    echo "    LlamaStack (3.4-) or OGX (3.5+) distribution management"
    echo ""
